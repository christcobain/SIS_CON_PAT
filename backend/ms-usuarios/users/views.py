...
class UserViewSet(ViewSet):
    ...
    @extend_schema(
        ...
        description="Filtros dinámicos combinables.",
        parameters=[
            OpenApiParameter("search",             OpenApiTypes.STR,  description="Buscar por nombre, apellido o DNI"),
            ...
            OpenApiParameter("role",               OpenApiTypes.INT,  description="ID de rol"),
            OpenApiParameter("dependencia",        OpenApiTypes.INT,  description="ID de dependencia"),
            OpenApiParameter("empresa_id",         OpenApiTypes.INT,  description="ID de empresa/corte"),  # ← nuevo
            OpenApiParameter("sedes",              OpenApiTypes.INT,  many=True,
                             description="IDs de sedes. Ej: ?sedes=1&sedes=2"),
            OpenApiParameter("cargo",              OpenApiTypes.STR,  description="Cargo (icontains)"),
            OpenApiParameter("created_between",    OpenApiTypes.STR,
                             description="Rango de fechas. Formato: YYYY-MM-DD,YYYY-MM-DD"),
            OpenApiParameter("fecha_baja_between", OpenApiTypes.STR,
                             description="Rango fecha baja. Formato: YYYY-MM-DD,YYYY-MM-DD"),
        ],
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter("search",             OpenApiTypes.STR,  description="Buscar por nombre, apellido o DNI"),
            OpenApiParameter("es_usuario_sistema", OpenApiTypes.BOOL, description="Tipo de usuario"),
            OpenApiParameter("is_active",          OpenApiTypes.BOOL, description="Estado"),
            OpenApiParameter("role",               OpenApiTypes.INT,  description="ID de rol"),
            OpenApiParameter("sede",               OpenApiTypes.INT,  description="ID de sede"),
            OpenApiParameter("empresa_id",         OpenApiTypes.INT,  description="ID de empresa/corte"),  # ← nuevo
            OpenApiParameter("dependencia",        OpenApiTypes.INT,  description="ID de dependencia"),
        ],
        responses={200: UserListSerializer(many=True)},
    ...
    @extend_schema(
        ...
        description=(
            "Crea un nuevo usuario consultando BDEmpleados por DNI. "
            "Los datos personales (nombre, cargo, empresa, etc.) se obtienen "
            "automáticamente del sistema de RRHH. "
            "Si es usuario de sistema, se generan credenciales con contraseña = DNI."
        ),
    ...
    def create(self, request):
        ...
        user = UserService.create_user(serializer.validated_data, sede_ids=sede_ids,created_by=creado_por)
    ...
    @extend_schema(
        ...
        description="Actualiza rol, sedes, dependencia, empresa o tipo de usuario.",
    ...
    @extend_schema(tags=['Users'], summary="Activar usuario",
                   responses={200: OpenApiTypes.OBJECT})
    @action(detail=True, methods=["patch"])
    def activate(self, request, pk=None):
        ...
        return Response(user, status=status.HTTP_200_OK)
    @extend_schema(tags=['Users'], summary="Desactivar usuario",
                   responses={200: OpenApiTypes.OBJECT})
    @action(detail=True, methods=["patch"])
    def deactivate(self, request, pk=None):
