...
class MantenimientoService:
    ...
    @staticmethod
    @transaction.atomic
    def crear(data: Dict[str, Any],usuario_realiza_id: int, sede_id: int,modulo_id: Optional[int],role: str) -> Dict[str, Any]:
    ...
    def enviar_a_aprobacion(pk: int,usuario_id: int,detalles_tecnicos: List[Dict],role: str) -> Dict[str, Any]:
        ...
        for item in detalles_tecnicos:
            ...
            if not ef_id:
                raise ValidationError(
                    f'Debe indicar el estado de funcionamiento final para el bien id={bien_id}.'
                )
            if not CatEstadoFuncionamiento.objects.filter(pk=ef_id).exists():
                raise ValidationError(f'Estado de funcionamiento id={ef_id} no existe.')
    ...
    @staticmethod
    def listar_pendientes_aprobacion(user_id: int, role: str, sede_id: int, modulo_id: int, token: str):
    ...
    @staticmethod
    @transaction.atomic
    def aprobar(pk: int,aprobador_id: int,role: str,sede_id: int,modulo_id: Optional[int],observacion: str = '',cookie: str = '') -> Dict[str, Any]:
        m = MantenimientoService._get_or_404(pk)
    ...
    @staticmethod
    @transaction.atomic
    def devolver(pk: int,aprobador_id: int,role: str,sede_id: int,modulo_id: Optional[int],motivo: str) -> Dict[str, Any]:
        m = MantenimientoService._get_or_404(pk)
        if m.estado_mantenimiento != 'PENDIENTE_APROBACION':
    ...
    def obtener_documento(pk: int, cookie: str = '') -> bytes:
        ...
        if m.estado_mantenimiento not in ('APROBADO', 'ATENDIDO'):
            raise ValidationError('El documento solo está disponible cuando el estado es APROBADO o ATENDIDO.')
    ...
    def subir_pdf_firmado(pk: int,archivo,usuario_id: int,role: str,cookie: str) -> Dict[str, Any]:
        ...
        if m.estado_mantenimiento != 'APROBADO':
            raise ValidationError('Solo se puede subir el documento firmado cuando el estado es APROBADO.')
