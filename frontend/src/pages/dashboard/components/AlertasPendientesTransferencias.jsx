...
function TarjetaPendiente({ t, role, sedeId, onDetalle, onAprobado, user, acciones }) {
  ...
  const hayAccionPrimaria = puedeAprobarAdmin || puedeAprobarSalida || puedeAprobarEntrada || puedeConfirmarRecepcion || puedeSubirActa || puedeRetornoSalida || puedeRetornoEntrada;
  ...
  const getPaso = () => {
    if (puedeAprobarAdmin) return { paso: '①', desc: 'Requiere aprobación de Admin Sede', color: 'var(--color-primary)' };
      ...
      <div className="card p-4 hover:shadow-md transition-shadow" style={{ borderLeft: puedeSubirActa ? '3px solid rgb(124 58 237 / 0.5)' : soloInformativoConformidad ? '3px solid rgb(37 99 235 / 0.3)' : undefined }}>
        ...
        <div className="flex items-center gap-2 flex-wrap border-t pt-3" style={{ borderColor: 'var(--color-border-light)' }}>
          ...
          {puedeConfirmarRecepcion && (
            <ActionBtn icon="front_hand" label="Confirmar Usuario Final" color="#1d4ed8" bgColor="rgb(37 99 235 / 0.08)" borderColor="rgb(37 99 235 / 0.3)" disabled={busy} onClick={() => ejecutar(transferenciasService.confirmarRecepcion, t.id, {})} />
          )}
