...
export default function Login() {
  ...
  // Control del modal ChangePassword
  const [cpOpen,     setCpOpen]     = useState(false);
  const [cpMode,     setCpMode]     = useState('voluntary');   // 'voluntary' | 'expired' | 'warning'
  const [cpUsername, setCpUsername] = useState('');            // username para flujo expired

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await login(username, password);
      // login navega al dashboard o llama a openChangePassword si es warning/expired
    } catch (err) {
      // El error de expired llega por el callback del useAuth (ver abajo)
    }
  };

  // Llamado desde useAuth cuando la clave está expirada
  // Login expone esta función al hook a través de un evento personalizado
  ...
  // useAuth necesita poder abrir el modal desde Login — usamos un event del DOM
  // El hook emite: window.dispatchEvent(new CustomEvent('sisconpat:openChangePassword', { detail: { mode, username } }))
  useState(() => {
    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-12" style={bgStyle}>
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
          ...
          {/* Footer tarjeta — botón cambiar contraseña */}
