...
function AccionesFila({ item, userId, userRole, onVerDetalle, onGestionar, onCancelar, onDescargarPDF }) {
  ...
  const tienePdfFirmado = !!item.pdf_firmado_path;

  // Misma lógica que ModalDetalleBaja
  const puedeDescargarSinFirma = esAtendido && yaAprobado && tienePdfBase && !tienePdfFirmado && esElaborador;
  const puedeDescargarFirmado  = esAtendido && tienePdfFirmado; // cualquiera con acceso
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      {/* Aprobar / Devolver — aprobador */}
      {item.estado_baja === 'PENDIENTE_APROBACION' && canApprove && (
        <>
          <button
            onClick={() => onGestionar(item, 'aprobar')}
      ...
      {/* Descargar PDF sin firma — elaborador, aprobado pero sin firmado */}
      {puedeDescargarSinFirma && (
        <button
          onClick={() => onDescargarPDF(item.id, false)}
      ...
      {/* Descargar PDF firmado — cualquiera con acceso, una vez firmado */}
      {puedeDescargarFirmado && (
        <button
          onClick={() => onDescargarPDF(item.id, true)}
      ...
      <Can perform="ms-bienes:bajas:delete_baja">
        {!['ATENDIDO', 'CANCELADO'].includes(item.estado_baja) && esElaborador && (
          <button
        ...
        <table className="table w-full">
          <thead>
            <tr className="bg-surface-alt/50 border-b border-border">
              {['Informe', 'Sede Elabora', 'Estado', 'N° Bienes', 'Registro', 'Aprobación Final', 'Doc.', 'Acciones'].map((h) => (
                <th
                  key={h}
                  className={`px-4 py-4 text-[10px] uppercase font-black tracking-widest text-faint ${
                    h === 'Acciones' || h === 'Doc.' ? 'text-right' : 'text-left'
                  }`}
                >
                  {h}
                </th>
          ...
          <tbody className="divide-y divide-border">
            {items.map((m) => {
              ...
              const tieneFirmado  = !!m.pdf_firmado_path;
              const tieneBase     = !!m.pdf_path;

              return (
                <tr key={m.id} className="hover:bg-surface-alt/30 transition-colors group">
                  <td className="px-4 py-3">
                    <p className="text-xs font-bold text-primary">{m.numero_informe}</p>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className="inline-flex items-center text-[10px] font-bold px-2.5 py-1 rounded-xl"
                  ...
                  <td className="px-4 py-3">
                    {m.estado_baja === 'CANCELADO' ? (
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-body">{m.nombre_coordsistema || 'Aprobador'}</p>
                        <p className="text-[9px] text-faint">{fmtT(m.fecha_aprobacion)}</p>
                      </div>
                    ) : m.estado_baja === 'DEVUELTO' ? (
                      <span className="text-amber-600 font-bold text-[10px] uppercase">Devuelto</span>
                    ) : (
                      <span className="text-faint text-[10px]">Pendiente</span>
                    )}
                  </td>
                  {/* Columna estado del documento */}
                  <td className="px-4 py-3 text-right">
                    {m.estado_baja === 'ATENDIDO' && tieneBase && (
                      tieneFirmado ? (
