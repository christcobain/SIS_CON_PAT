...
function FInput({ value, onChange, placeholder }) {
  return (
    <div className="relative flex-1 min-w-[180px]">
      <Icon name="search" style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none', color: value ? 'var(--color-primary)' : 'var(--color-text-faint)' }} />
...
export default function MantenimientosFiltros({ filtros, onChange, onLimpiar, sedes = [] }) {
  const hayFiltros = filtros.estado || filtros.sede_id || filtros.search;

  return (
    <div className="card p-4 flex items-center gap-3 flex-wrap">
      <FInput value={filtros.search || ''} onChange={v => onChange('search', v)} placeholder="Buscar por N° orden..." />

      <FSelect value={filtros.estado || ''} onChange={v => onChange('estado', v)}>
        <option value="">Todos los estados</option>
        {ESTADOS.map(e => <option key={e.value} value={e.value}>{e.label}</option>)}
      </FSelect>

      {sedes.length > 0 && (
        <FSelect value={filtros.sede_id || ''} onChange={v => onChange('sede_id', v)}>
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={() => onChange('misMantenimientos', !filtros.misMantenimientos)}
          className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest px-3 py-2.5 rounded-xl transition-all"
          style={{
            background: filtros.misMantenimientos ? 'rgb(127 29 29 / 0.08)' : 'var(--color-surface)',
            border: `1px solid ${filtros.misMantenimientos ? 'rgb(127 29 29 / 0.3)' : 'var(--color-border)'}`,
            color: filtros.misMantenimientos ? 'var(--color-primary)' : 'var(--color-text-muted)',
          }}>
          <Icon name={filtros.misMantenimientos ? 'person' : 'public'} />
          {filtros.misMantenimientos ? 'Mis órdenes' : 'Todos'}
        </button>
